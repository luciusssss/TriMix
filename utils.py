    ...
    **generation_kwargs
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        if return_logits_for_analysis:
            batch_outputs, analysis_data = model.generate(
            ...
            # print("Got analysis data for a batch of size", len(batch_prompts))
            # print(analysis_data)
            # convert defaultdict to dict
            analysis_data = {k: v for k, v in analysis_data.items()}
            all_returned_analysis_data.append(analysis_data)
        else:
    ...
    return generations if not return_logits_for_analysis else (generations, all_returned_analysis_data)
...
newly-added
'''
def load_trimix_model_and_tokenizer(
    base_model_name_or_path: str,
    expert_model_name_or_path: str,
    antiexpert_model_name_or_path: str = None,
    device_map: str = "auto",
    ...
    from transformers import AutoTokenizer
    from dexperts import TriMixQwen, TriMixGemma
    ...
    if "qwen" in base_model_name_or_path.lower() or "llama" in base_model_name_or_path.lower():
        model = TriMixQwen(
            base_model_name_or_path=base_model_name_or_path,
            expert_model_name_or_path=expert_model_name_or_path,
            antiexpert_model_name_or_path=antiexpert_model_name_or_path,
    ...
    elif  'gemma' in base_model_name_or_path.lower():
        model = TriMixGemma(
            base_model_name_or_path=base_model_name_or_path,
            expert_model_name_or_path=expert_model_name_or_path,
            antiexpert_model_name_or_path=antiexpert_model_name_or_path,
        ...
        raise NotImplementedError("Only support Qwen, Llama and Gemma model for TriMix")
